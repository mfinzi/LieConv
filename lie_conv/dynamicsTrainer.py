...
class HLieResNet(LieResNet, HNet):
    def __init__(self, d=2, sys_dim=2, bn=False, num_layers=4, group=T(2), k=384, knn=False, nbhd=100, mean=True,
                 center=True, **kwargs):
...
class FLieResnet(LieResNet):  # An (equivariant) lieConv network that models the dynamics directly
    ...
    def forward(self, t, z, sysP, wgrad=True):
        ...
        p = z[:, D // 2:].reshape(*m.shape, -1)
        q = q - q.mean(1,
                       keepdims=True)  # (m.unsqueeze(-1)*q).sum(dim=1,keepdims=True)/m.sum(1,keepdims=True).unsqueeze(-1)
        # q,sys_params = x
        bs, n, d = q.shape
        mask = ~torch.isnan(q[..., 0])
