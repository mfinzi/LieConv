...
class MolecLieResNet(LieResNet):
    ...
    # def add_features_to(self,dataset):
    #     processing_BS = 1000 # Feature processing batchsize, unrelated to training batch size
    #     ns = self.liftsamples
    #     mb = dataset.data
    #     N, n, d, c = *mb['charges'].shape, SE3.embed_dim, self.ch0
    #     mb['lifted_pairs'] = torch.zeros(N,n,ns,n,ns,d)
    #     mb['lifted_vals'] = torch.zeros(N,n,ns,c)
    #     mb['lifted_mask'] = torch.zeros(N,n,ns)>1
    #     with torch.no_grad():
    #         for i in range(int(np.ceil(N/processing_BS))):
    #             mbslice = slice(i*processing_BS,(i+1)*processing_BS)
    #             charges = mb['charges'][mbslice] / self.charge_scale
    #             c_vec = torch.stack([torch.ones_like(charges),charges,charges**2],dim=-1) # 
    #             one_hot_charges = (mb['one_hot'][mbslice][:,:,:,None]*c_vec[:,:,None,:]).float().reshape(*charges.shape,-1)
    #             atomic_coords = mb['positions'][mbslice].float()
    #             atom_mask = mb['charges'][mbslice]>0
    #             x = (atomic_coords.cuda(), one_hot_charges.cuda(), atom_mask.cuda())
    #             assert not torch.any(torch.isnan(atomic_coords)), \
    #                 f"nans in coords {torch.isnan(atomic_coords).sum()}"
    #             lifted_a,lifted_v,lifted_m = SE3.lift(x,nsamples=ns,alpha=.5)
    #             paired_a = SE3.elems2pairs(lifted_a) #(bs,n*ns,d) -> (bs,n*ns,n*ns,d)
    #             nans_discard = torch.isnan(paired_a)|torch.isinf(paired_a)
    #             print(f"discarded {nans_discard.sum()} nans or inf")
    #             paired_a = torch.where(nans_discard,torch.zeros_like(paired_a),paired_a)
    #             # TODO fix the nan coming from coscc in SE3.log
    #             BS = paired_a.shape[0]
    #             mb['lifted_pairs'][mbslice] = paired_a.reshape(BS,n,ns,n,ns,d).cpu()
    #             mb['lifted_vals'][mbslice] = lifted_v.reshape(BS,n,ns,lifted_v.shape[-1]).cpu()
    #             mb['lifted_mask'][mbslice] = lifted_m.reshape(BS,n,ns).cpu()

    #             assert not torch.any(torch.isnan(mb['lifted_pairs'][mbslice])), \
    #                 f"nans in paired_a {torch.isnan(mb['lifted_pairs'][mbslice]).sum()}"
    #             assert not torch.any(torch.isinf(mb['lifted_pairs'][mbslice])), \
    #                 f"infs in paired_a {torch.isinf(mb['lifted_pairs'][mbslice]).sum()}"
                
    #             print(f"completed {BS+i*processing_BS}")
